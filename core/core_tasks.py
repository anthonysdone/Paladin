...
def load_hex(name, size):
    ...
    try:
        ...
        print(f"Failed to open HEX file: {name}")
...
if __name__ == "__main__":
    num_cores = 1
    program = "bubblesort.hex"

    print(f"Instantiating core...")
    sim, outputs = rv32i_5stage("bubblesort.hex")
    ...
    for cycle in range(1001): 
        ...
        if cycle % 10 == 0:
            ...
            print(f"RA: 0x{outputs["regfile"].val[1]:08x}")
            print(f"SP: 0x{outputs["regfile"].val[2]:08x}")
            print(f"DMem[0]: {outputs["dmem"].val[0]}")
            print(f"DMem[1]: {outputs["dmem"].val[1]}")